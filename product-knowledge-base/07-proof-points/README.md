...
Ask Context Builder to codify it:
```
"Codify this case study into our proof points library: [URL]"
```

**Option 2: From a customer stories page**
Give Context Builder the URL where all case studies are listed:
```
"Codify all the case studies from [URL] into our proof points library"
```

**Option 3: From a document or notes**
Paste or attach the case study content and ask Context Builder to structure it.

### How Other Agents Use Case Studies

- **Asset Builder** checks case studies for approved quotes and testimonials when generating ads, emails, landing pages, and briefs
- **Alignment Auditor** verifies that customer quotes used in content come from approved case studies
...
Ask Context Builder to codify them:
```
...
- **Asset Builder** checks data claims for substantiated proof points before using any quantitative claim in content
- **Alignment Auditor** verifies that any data claim in generated content exists in the data claims file and is `active`
- **Context Builder** adds new claims when codifying case studies (metrics from case studies become data claims)
...
- `01-segment-context/[segment]/positioning-strategy.md` — Proof points and evidence section
- `01-segment-context/[segment]/messaging-framework.md` — Copy blocks and claims
